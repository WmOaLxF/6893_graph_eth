...
def tune_mnist_pbt(num_samples=10, num_epochs=10, gpus_per_trial=0, checkpoint_dir=None):
...
if __name__ == '__main__':
  ...
  bucket.blob(data_file_name).download_to_filename(data_file_name)
  results = tune_mnist_pbt(**tune_config)
  results_metrics = results.get_best_result()
  init_config = results_metrics.metrics['config']
  torch.save(init_config, init_config_name)
  bucket.blob(init_config_name).upload_from_filename(init_config_name)